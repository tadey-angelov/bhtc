...
static int dequeue(void) {
	...
	xSemaphoreTake(lock_hnd, portMAX_DELAY);
	result = queue_buffer[(out++) & (SIZE - 1u)];
	xSemaphoreGive(lock_hnd);
...
void app_main(void) {
	...
	countsem_hnd = xSemaphoreCreateCountingStatic(SIZE, 0u, &countsem);
	(void)countsem_hnd;
	
	spacesem_hnd = xSemaphoreCreateCountingStatic(SIZE, SIZE, &spacesem);
	(void)spacesem_hnd;
